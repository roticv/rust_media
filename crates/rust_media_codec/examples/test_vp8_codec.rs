...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Testing VP8 Decoder and Encoder\n");

    // Test 1: Decode VP8 WebM file
    println!("=== Test 1: Decoding VP8 WebM ===");
    let input_file = File::open("test_assets/test_vp8.webm")?;
    ...
    // Create VP8 decoder
    let mut decoder = Vp8Decoder::new(stream_info.clone())?;
    ...
    println!("=== Test 2: VP8 Decode -> Encode Roundtrip ===");
    ...
    let mut decoder = Vp8Decoder::new(stream_info.clone())?;
    let mut encoder = Vp8Encoder::new(stream_info.clone())?;

    // Create output muxer
    let output_file = File::create("/tmp/test_vp8_roundtrip.webm")?;
    ...
    let output_size = std::fs::metadata("/tmp/test_vp8_roundtrip.webm")?.len();
    println!("Output file size: {} bytes", output_size);

    println!("\n🎉 All VP8 tests passed!");
