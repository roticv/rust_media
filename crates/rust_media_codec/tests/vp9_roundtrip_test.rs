//! VP9 encode → decode roundtrip integration test
//!
//! Generates a sequence of solid-color video frames with unique luma values,
//! encodes them with VP9, decodes them back, and verifies that each decoded
...
fn make_stream_info(width: usize, height: usize, time_base_den: u32) -> StreamInfo {
    ...
    StreamInfo::new(0, MediaType::Video, "vp9".to_string())
        .with_params(StreamParams::Video(video_params))
        .with_time_base(1, time_base_den)
}

#[test]
fn vp9_color_ramp_roundtrip_preserves_per_frame_luma() {
    println!("\n=== VP9 Color Ramp Roundtrip Test ===\n");
    ...
    // ------------------------------------------------------------------
    // 2. Encode with Vp9Encoder
    // ------------------------------------------------------------------
    let stream_info = make_stream_info(WIDTH, HEIGHT, TIME_BASE_DEN);
    let mut encoder = Vp9Encoder::with_bitrate(stream_info.clone(), BITRATE)
    ...
    println!("Encoded into {} VP9 packets", encoded_packets.len());
    ...
    assert!(
        ...
        "first VP9 packet should be a keyframe"
    );

    // ------------------------------------------------------------------
    // 3. Decode with Vp9Decoder
    // ------------------------------------------------------------------
    let mut decoder = Vp9Decoder::new(stream_info).expect("failed to create VP9 decoder");
