//! H.264 encode → decode roundtrip integration test
//!
//! Encodes color-ramp frames with x264 (gpl-x264 feature) and decodes them
//! back with the rust_h264 software decoder. Verifies frame count and per-frame
//! luma values are preserved within tolerance.
//!
//! This test only runs when the `gpl-x264` feature is enabled, since the
//! H.264 encoder requires the GPL x264 library.
...
use rust_media_codec::{H264Decoder, X264Encoder};
use rust_media_core::{
...
const TIME_BASE_DEN: u32 = 30;
const BITRATE: u64 = 1_000_000;
...
fn make_stream_info(width: usize, height: usize, time_base_den: u32) -> StreamInfo {
    ...
    StreamInfo::new(0, MediaType::Video, "h264".to_string())
        .with_params(StreamParams::Video(video_params))
        .with_time_base(1, time_base_den)
}

#[test]
fn h264_color_ramp_roundtrip_preserves_per_frame_luma() {
    println!("\n=== H.264 Color Ramp Roundtrip Test ===\n");
    ...
    // ------------------------------------------------------------------
    // 2. Encode with X264Encoder
    // ------------------------------------------------------------------
    let stream_info = make_stream_info(WIDTH, HEIGHT, TIME_BASE_DEN);
    let mut encoder = X264Encoder::with_bitrate(stream_info.clone(), BITRATE)
        .expect("failed to create x264 encoder");
    ...
    println!("Encoded into {} H.264 packets", encoded_packets.len());
    ...
    // ------------------------------------------------------------------
    // 3. Decode with H264Decoder (rust_h264, accepts Annex B input)
    // ------------------------------------------------------------------
    let mut decoder = H264Decoder::new(stream_info).expect("failed to create H.264 decoder");
    ...
    // H.264 with VBR @ 1 Mbps on flat-color frames should be near-perfect.
